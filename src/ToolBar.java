...
public class ToolBar extends JPanel implements ActionListener
{
    ...
    private MandelbrotControl mPane;
     ...
     */
    public ToolBar(int w, int h, MandelbrotControl mPane, JFrame parent, boolean docked)
    {
        this.docked = docked;
        this.setSize(w, h);
        this.setLayout(null);
        ...
        maxField.setValue(mPane.getMAX_VAL());

        colorSlider = new JSlider(JSlider.HORIZONTAL, 1, 10, mPane.getColor());
        ...
        zoomField.setValue(mPane.getZoom());
        ...
        wField.setValue(mPane.getImgW());

        hField = new JFormattedTextField(format);
        hField.setValue(mPane.getImgH());
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        {
            mPane.setColor(Math.abs(colorSlider.getValue()));
            mPane.setMAX_VAL(Math.abs(Integer.parseInt(maxField.getText())));
            mPane.setZoom(Math.abs(Integer.parseInt(zoomField.getText())));
            mPane.render();
        }
        ...
        else if(event.getSource() == save)
        {
            try {
                ...
                ImageIO.write((RenderedImage) mPane.getImg(), "png", f);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        ...
        {
            mPane.setImgW(Math.abs(Integer.parseInt(wField.getText())), Math.abs(Integer.parseInt(hField.getText())));
            if(docked)
            {
                parent.setSize(mPane.getImgW(), mPane.getImgH()+210);
                this.setLocation(0, mPane.getHeight()+5);
                initialize();
            }
            else
            {
                parent.setSize(mPane.getImgW(), mPane.getImgH()+parent.getInsets().top+parent.getInsets().bottom);
            }
        }
        //reset the view
